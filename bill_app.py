...
def render_sidebar(menu):
    """Render the sidebar with meal selection options."""
...
def render_main_content():
    ...
    week, day, meal = render_sidebar(menu)
    ...
    with st.form("quantities_form"):
        if selected_dishes:
            ...
            num_cols = min(len(selected_dishes), 3)  # Max 3 columns for readability
    ...
    if submit and selected_dishes:
        ...
        if bill_items:
            ...
            download = st.download_button(
                ...
                csv_data,
                f"billyourbite_summary_{week}_{day}_{meal}.csv",
                "text/csv",
                help="Download your bill as a CSV file"
            )
            
            if download:
                st.success("📄 Your bill summary has been downloaded!")
